...
namespace Astrelis::OpenGL
{
    ...
    Ptr<GLShaderProgram> GLShaderProgram::Create(std::string name, std::string_view vertexSource, std::string_view fragmentSource)
    {
        ...
        {
            ASTRELIS_LOG_ERROR("Vertex shader compilation failed: {0}", vertexShader.GetInfoLog());
            return nullptr;
        }
        ...
        {
            ASTRELIS_LOG_ERROR("Fragment shader compilation failed: {0}", fragmentShader.GetInfoLog());
            return nullptr;
        }
        ...
        if (!program->Link())
        {
            ASTRELIS_LOG_ERROR("Shader program linking failed: {0}", program->GetInfoLog());
            return nullptr;
        }
