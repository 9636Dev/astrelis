...
#include "Astrelis/Core/Utils/Function.hpp"
    ...
    #define GLCall(x)                                                   \
        ...
        {                                                               \
        default:                                                        \
            ::Astrelis::OpenGL::GL::GLCallHelper(__FILE__, __LINE__, #x); \
            x;                                                          \
        }
    #define GLCallR(x)                                                  \
        [&]() {                                                         \
...
namespace Astrelis::OpenGL
{
    ...
    void GL::GLCallHelper(std::string_view file, int line, std::string_view function)
    {
        ...
        while ((error = glGetError()) != GL_NO_ERROR)
        {
            ...
            ASTRELIS_CORE_LOG_ERROR("OpenGL Error: {0} in {1} at line {2} in function {3}", errorString, file, line,
                                  function);
        }
    }
    ...
    {
        ASTRELIS_UNUSED(userParam);
        ASTRELIS_UNUSED(length);
        ...
        case GL_DEBUG_SEVERITY_HIGH:
            ASTRELIS_CORE_LOG_FATAL("[OpenGL High Severity Error] {0} ({1}) at {2}:{3} ({4}) [{5} {6}]", message, errId,
                                  s_CallLocation.File, s_CallLocation.Line, s_CallLocation.Function, sourceString,
                                  typeString);
            break;
        case GL_DEBUG_SEVERITY_MEDIUM:
            ASTRELIS_CORE_LOG_ERROR("[OpenGL Medium Severity Error] {0} ({1}) at {2}:{3} ({4}) [{5} {6}]", message, errId,
                                  s_CallLocation.File, s_CallLocation.Line, s_CallLocation.Function, sourceString,
                                  typeString);
            break;
        case GL_DEBUG_SEVERITY_LOW:
            ASTRELIS_CORE_LOG_WARN("[OpenGL Low Severity Error] {0} ({1}) at {2}:{3} ({4}) [{5} {6}]", message, errId,
                                 s_CallLocation.File, s_CallLocation.Line, s_CallLocation.Function, sourceString,
                                 typeString);
            break;
        case GL_DEBUG_SEVERITY_NOTIFICATION:
#ifdef ASTRELIS_GL_VERBOSE
            if (type == GL_DEBUG_TYPE_PERFORMANCE)
            {
                ASTRELIS_CORE_LOG_TRACE("[OpenGL Performance Notification] {0}", message);
                break;
            }
            ASTRELIS_CORE_LOG_INFO("[OpenGL Notification] {0} ({1}) at {2}:{3} ({4}) [{5} {6}]", message, errId,
                                 s_CallLocation.File, s_CallLocation.Line, s_CallLocation.Function, sourceString,
                                 typeString);
#endif
            (void)message;
