...
namespace Astrelis::Vulkan {
    struct Version {
        ...
        std::uint32_t GetMajor() const {
            return VK_VERSION_MAJOR(VulkanVersion);
        }

        std::uint32_t GetMinor() const {
            return VK_VERSION_MINOR(VulkanVersion);
        }

        std::uint32_t GetPatch() const {
            return VK_VERSION_PATCH(VulkanVersion);
        }
    };
    ...
    public:
        ...
        [[nodiscard]] bool Init(CString appName, Version appVersion, Version apiVersion,
