...
namespace Astrelis::Vulkan {
    ...
    void Ext::Init(VkInstance instance) {
        ...
        ASTRELIS_CORE_ASSERT(
            CreateDebugUtilsMessengerEXT, "Failed to load vkCreateDebugUtilsMessengerEXT");
        DestroyDebugUtilsMessengerEXT = reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(
            vkGetInstanceProcAddr(instance, "vkDestroyDebugUtilsMessengerEXT"));
        ASTRELIS_CORE_ASSERT(
            DestroyDebugUtilsMessengerEXT, "Failed to load vkDestroyDebugUtilsMessengerEXT");
        CmdBeginDebugUtilsLabelEXT = reinterpret_cast<PFN_vkCmdBeginDebugUtilsLabelEXT>(
            vkGetInstanceProcAddr(instance, "vkCmdBeginDebugUtilsLabelEXT"));
        ASTRELIS_CORE_ASSERT(
            CmdBeginDebugUtilsLabelEXT, "Failed to load vkCmdBeginDebugUtilsLabelEXT");
        ...
        ASTRELIS_CORE_ASSERT(
            CmdInsertDebugUtilsLabelEXT, "Failed to load vkCmdInsertDebugUtilsLabelEXT");
    }
    ...
    void Ext::BeginDebugLabel(
        VkCommandBuffer commandBuffer, const char* label, std::array<float, 4> color) {
    ...
    void Ext::EndDebugLabel(VkCommandBuffer commandBuffer) {
        CmdEndDebugUtilsLabelEXT(commandBuffer);
    }

    void Ext::InsertDebugLabel(
        VkCommandBuffer commandBuffer, const char* label, std::array<float, 4> color) {
        VkDebugUtilsLabelEXT labelInfo = {};
        labelInfo.sType                = VK_STRUCTURE_TYPE_DEBUG_UTILS_LABEL_EXT;
        labelInfo.pLabelName           = label;
