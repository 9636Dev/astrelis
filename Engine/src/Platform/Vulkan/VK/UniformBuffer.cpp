...
namespace Astrelis::Vulkan
{
    bool UniformBuffer::Init(RefPtr<GraphicsContext>& context, std::uint32_t size)
    {
        ...
        for (Buffer& buffer : m_Buffers)
        {
            ...
            if (vkMapMemory(ctx->m_LogicalDevice.GetHandle(), buffer.m_Memory, 0, bufferSize, 0,
                            &buffer.m_MappedMemory) != VK_SUCCESS)
            {
    ...
    void UniformBuffer::SetData(RefPtr<GraphicsContext>& context,
                                const void* data,
                                std::uint32_t size,
                                std::uint32_t offset)
    {
        ...
        std::memcpy(
            static_cast<std::uint8_t*>(m_Buffers[context.As<VulkanGraphicsContext>()->m_CurrentFrame].m_MappedMemory) +
                offset,
            data, size);
    }
} // namespace Astrelis::Vulkan
