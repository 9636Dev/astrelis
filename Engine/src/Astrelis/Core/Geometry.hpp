...
namespace Astrelis {
    ...
    template<Types::Arithmetic T> struct Dimension3D {
        ...
        constexpr Dimension3D(T width, T height, T depth) noexcept
    ...
    template<Types::Arithmetic T> struct Rect2D {
        ...
        constexpr Rect2D(Point2D<T> point, Dimension2D<T> dimensions) noexcept
            : Position(point), Size(dimensions) {
        }

        constexpr Rect2D(T posX, T posY, T width, T height) noexcept
    ...
    template<Types::Arithmetic T> struct Rect3D {
        ...
        constexpr Rect3D(Point3D<T> point, Dimension3D<T> dimensions) noexcept
            : Position(point), Size(dimensions) {
        }

        constexpr Rect3D(T posX, T posY, T posZ, T width, T height, T depth) noexcept
            : Position(posX, posY, posZ), Size(width, height, depth) {
        }
        ...
        [[nodiscard]] T Depth() const noexcept {
            return Size.Depth;
        }
    };
