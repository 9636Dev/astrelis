...
namespace Astrelis {
    ImGuiLayer::ImGuiLayer(RefPtr<ImGuiBackend> backend)
        : Layer("ImGuiLayer"), m_Backend(std::move(backend)) {
    }
    ...
    void ImGuiLayer::OnEvent(Event& event) {
        if (m_BlockEvents) {
            ...
            event.Handled |=
                event.IsInCategory(EventCategory::Keyboard) && imguiIo.WantCaptureKeyboard;
        }
    ...
    void ImGuiLayer::Resize(ViewportResizedEvent& event) {
        m_Backend->Resize(static_cast<std::int32_t>(event.GetWidth()),
