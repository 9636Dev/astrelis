...
namespace Astrelis {
            ...
            if (m_Channels == 3) {
                for (int y = 0; y < m_Height; y++) {
                    for (int x = 0; x < m_Width; x++) {
                        m_Data[4 * (y * m_Width + x) + 0] =
                            static_cast<std::byte>(data[3 * (y * m_Width + x) + 0]);
                        m_Data[4 * (y * m_Width + x) + 1] =
                            static_cast<std::byte>(data[3 * (y * m_Width + x) + 1]);
                        m_Data[4 * (y * m_Width + x) + 2] =
                            static_cast<std::byte>(data[3 * (y * m_Width + x) + 2]);
    ...
    double Covariance(const std::vector<double>& values1, const std::vector<double>& values2,
    ...
    float InMemoryImage::Similarity(const InMemoryImage& other, CompareMethod method) const {
        ...
        double contrast =
            (2.0 * std::sqrt(variance1) * std::sqrt(variance2) + c2) / (variance1 + variance2 + c2);
        double structure =
            (covariance + c2 / 2.0) / (std::sqrt(variance1) * std::sqrt(variance2) + c2 / 2.0);
