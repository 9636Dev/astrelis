...
namespace Astrelis {
    BaseRenderer::BaseRenderer(RefPtr<Window> window, Rect2Di viewport)
        : m_Window(std::move(window)), m_Context(m_Window->GetGraphicsContext()),
    ...
    void BaseRenderer::InternalBeginFrame() {
        ...
        case RendererAPI::API::Vulkan:
            viewport = {0.0F, 0.0F, 0.0F, static_cast<float>(scissor.Width()),
                static_cast<float>(scissor.Height()), 1.0F};
        ...
        ASTRELIS_CORE_ASSERT(
            viewport.X() == 0.0F && viewport.Y() == 0.0F, "Viewport X and Y should be 0.0F");
    ...
    void BaseRenderer::ResizeViewport() {
