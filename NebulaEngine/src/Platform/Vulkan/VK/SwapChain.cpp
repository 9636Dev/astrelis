...
namespace Nebula::Vulkan
{
                         ...
                         VkSwapchainKHR oldSwapchain)
    {
        ...
        {
            NEBULA_CORE_LOG_INFO("Swap chain support details:");
            NEBULA_CORE_LOG_INFO("Surface formats:");
            for (const auto& format : swapChainSupport.formats)
            {
                NEBULA_CORE_LOG_INFO("\tFormat: {0}, Color space: {1}", Utils::VkFormatToString(format.format),
                                     Utils::VkColorSpaceToString(format.colorSpace));
            }

            NEBULA_CORE_LOG_INFO("Present modes:");
            for (const auto& mode : swapChainSupport.presentModes)
            {
                NEBULA_CORE_LOG_INFO("\t{0}", Utils::VkPresentModeToString(mode));
            }

            NEBULA_CORE_LOG_INFO("Surface format: {0}, Color space: {1}", Utils::VkFormatToString(surfaceFormat.format),
                                 Utils::VkColorSpaceToString(surfaceFormat.colorSpace));
            NEBULA_CORE_LOG_INFO("Present mode: {0}", Utils::VkPresentModeToString(presentMode));
            NEBULA_CORE_LOG_INFO("Extent: {0}x{1}", extent.width, extent.height);
        }
