...
namespace Nebula
{
    ...
    public:
        ...
        bool CanRead() const { return (std::filesystem::perms::owner_read & std::filesystem::status(m_Path).permissions()) != std::filesystem::perms::none; }
        bool CanWrite() const { return (std::filesystem::perms::owner_write & std::filesystem::status(m_Path).permissions()) != std::filesystem::perms::none; }
        bool CanExecute() const { return (std::filesystem::perms::owner_exec & std::filesystem::status(m_Path).permissions()) != std::filesystem::perms::none; }
        ...
        std::filesystem::path GetRelativePath(const File& other) const { return std::filesystem::relative(m_Path, other.GetPath()); }
