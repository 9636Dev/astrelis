...
namespace Nebula
{
    ...
    public:
        ...
        explicit TimeSpan(const TimeSpan<T2, Ratio2>& other) :
            m_Duration(std::chrono::duration_cast<std::chrono::duration<T, Ratio>>(other.m_Duration))
        {
        }

        template<typename T2, typename Ratio2> TimeSpan& operator=(const TimeSpan<T2, Ratio2>& other)
        {
            m_Duration = std::chrono::duration_cast<std::chrono::duration<T, Ratio>>(other.GetDuration());
            return *this;
        }
        ...
        template<typename T2, typename Ratio2> bool operator==(const TimeSpan<T2, Ratio2>& other) const
        {
            return m_Duration == std::chrono::duration_cast<std::chrono::duration<T, Ratio>>(other.GetDuration());
        }
        ...
        operator T() const { return m_Duration.count(); }
    private:
    ...
    public:
        ...
        static Seconds DeltaTime() { return Seconds(s_DeltaTime); }

        static Milliseconds DeltaTimeMillis() { return s_DeltaTime; }
