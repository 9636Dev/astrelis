...
# - CHANGE: Changes to source code that do not fit in any of the above categories (e.g. changing implementation details)
...
PREFIX_REGEX='^\[(BUG|FEAT|DOC|STYLE|REFACTOR|TEST|WIP|UPDATE|SUBMODULE|CI|CHORE|CHANGE)\] '
...
if [[ ! $COMMIT_MSG =~ $PREFIX_REGEX ]]; then
    ...
    echo "Valid prefixes include [BUG], [FEAT], [DOC], [STYLE], [REFACTOR], [TEST], [WIP], [UPDATE], [SUBMODULE], [CI], [CHORE], [CHANGE]."
    exit 1
fi
