...
#define EVENT_CLASS_TYPE(type) static constexpr EventType GetStaticType() noexcept { return EventType::type; }\
...
namespace Nebula
{
    ...
    inline constexpr std::uint32_t operator|(const EventCategory& lhs, const EventCategory& rhs) noexcept { return static_cast<std::uint32_t>(lhs) | static_cast<std::uint32_t>(rhs); }
    inline constexpr std::uint32_t operator|(std::uint32_t lhs, const EventCategory& rhs) noexcept { return lhs | static_cast<std::uint32_t>(rhs); }
    ...
    {
    public:
        explicit EventDispatcher(Event& p_Event) noexcept :
            m_Event(p_Event)
        {
